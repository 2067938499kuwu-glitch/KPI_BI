...
function DashboardSummaryCards({ roleId, summary, record }) {
  if (roleId === "employee") {
    const items = [
      { label: "最终评分状态", value: record?.finalScoreEffective ? "已生效" : "未完成", meta: record?.finalScoreEffectiveAt ?? "--" },
      { label: "周报异常数", value: record?.weekly.abnormalCount ?? 0, meta: "逾期 + 未提交" },
    ];
    return <section className="dashboard-summary-grid is-employee">{items.map((item) => <article key={item.label}><span>{item.label}</span><div><strong>{item.value}</strong></div><small>{item.meta}</small></article>)}</section>;
  }

  const total = summary.completed + summary.incomplete;
  const completionRate = total ? Math.round((summary.completed / total) * 1000) / 10 : 0;
  const items = [
    { label: "综合绩效完成度", value: summary.completed, unit: ` / ${total} 人`, meta: `已有 ${summary.completed} 人完成最终评分`, progress: completionRate },
    { label: "评分未完成", value: summary.incomplete, unit: "人", meta: "含审批中与退回", icon: ClipboardText },
    { label: "平均分", value: formatValue(summary.average), unit: "分", meta: "较上月整体保持稳定", icon: ChartBar },
    { label: "最高 / 最低", value: `${formatValue(summary.highest)} / ${formatValue(summary.lowest)}`, meta: "当前权限范围", icon: ArrowUpRight },
    { label: "周报按时提交率", value: formatValue(summary.weekly.onTimeRate, "%"), meta: "按时周数 ÷ 应提交周数", icon: CalendarBlank },
    { label: "周报逾期人数", value: summary.weekly.latePeople, unit: "人", meta: `${summary.weekly.missingPeople} 人存在未提交`, icon: UserMinus },
  ];

  return <section className="dashboard-summary-grid">{items.map((item, index) => {
    const Icon = item.icon;
    return <article className={index === 0 ? "is-progress" : ""} key={item.label}><div className="dashboard-summary-card__copy"><span>{item.label}</span><div><strong>{item.value}</strong>{item.unit ? <b>{item.unit}</b> : null}</div><small>{item.meta}</small></div>{item.progress !== undefined ? <div aria-label={`绩效完成度 ${item.progress}%`} className="dashboard-progress-ring" style={{ "--dashboard-progress": `${item.progress}%` }}><strong>{item.progress}%</strong></div> : <span className="dashboard-summary-card__mark" aria-hidden="true">{Icon ? <Icon size={17} weight="duotone" /> : <IdentificationCard size={17} weight="duotone" />}</span>}</article>;
  })}</section>;
}

function DashboardTrendPanel({ series }) {
  return <section className="dashboard-insight-card dashboard-trend-panel"><header><div><strong>绩效趋势对比</strong><span>月度平均分与评分完成率</span></div><span className="dashboard-insight-card__badge">近三月</span></header><div className="dashboard-chart-legend"><span><i className="is-score" />平均分</span><span><i className="is-completion" />完成率</span></div><div className="dashboard-bar-chart"><div className="dashboard-bar-chart__scale"><span>100</span><span>75</span><span>50</span><span>25</span></div><div className="dashboard-bar-chart__groups">{series.map((item) => <div className="dashboard-bar-chart__group" key={item.cycle}><div><i className="is-score" style={{ height: `${item.average}%` }} title={`平均分 ${item.average}`} /><i className="is-completion" style={{ height: `${item.completionRate}%` }} title={`完成率 ${item.completionRate}%`} /></div><strong>{item.cycle.slice(5)}月</strong></div>)}</div></div></section>;
}

function DashboardWeeklyPanel({ records }) {
  return <section className="dashboard-insight-card dashboard-weekly-panel"><header><div><strong>重点人员周报表现</strong><span>提交及时性与异常情况</span></div><span className="dashboard-insight-card__badge">本月</span></header><div className="dashboard-weekly-legend"><span><i className="is-normal" />按时</span><span><i className="is-late" />逾期</span><span><i className="is-missing" />未交</span></div><div className="dashboard-weekly-people">{records.slice(0, 5).map((record) => {
    const total = record.weekly.normal + record.weekly.late + record.weekly.missing;
    const normalWidth = total ? (record.weekly.normal / total) * 100 : 0;
    const lateWidth = total ? (record.weekly.late / total) * 100 : 0;
    const missingWidth = total ? (record.weekly.missing / total) * 100 : 100;
    return <article key={record.employeeId}><div className="dashboard-person"><i>{record.name.slice(0, 1)}</i><div><strong>{record.name}</strong><small>{record.department}</small></div></div><div className="dashboard-weekly-person__metric"><div aria-label={`${record.name}周报按时率 ${formatValue(record.weekly.onTimeRate, "%")}`}><i className="is-normal" style={{ width: `${normalWidth}%` }} /><i className="is-late" style={{ width: `${lateWidth}%` }} /><i className="is-missing" style={{ width: `${missingWidth}%` }} /></div><strong>{formatValue(record.weekly.onTimeRate, "%")}</strong></div></article>;
  })}{!records.length ? <div className="dashboard-mini-empty">暂无周报表现数据</div> : null}</div></section>;
}

function DashboardWeeklyOverview({ summary }) {
  const total = summary.requiredWeeks || 0;
  const rate = summary.onTimeRate ?? 0;
  return <section className="dashboard-insight-card dashboard-weekly-overview"><header><div><strong>周报提交概览</strong><span>本月提交完成度与异常分布</span></div><span className="dashboard-insight-card__badge">本月</span></header><div className="dashboard-weekly-overview__content"><div aria-label={`周报按时提交率 ${rate}%`} className="dashboard-weekly-overview__ring" style={{ "--dashboard-progress": `${rate}%` }}><span><strong>{rate}%</strong><small>按时提交率</small></span></div><div className="dashboard-weekly-overview__stats"><div className="dashboard-weekly-overview__total"><span>本月应提交周报</span><strong>{total}<small> 份</small></strong></div><div className="dashboard-weekly-overview__breakdown"><div><i className="is-normal" /><span>按时提交</span><strong>{summary.normal}</strong></div><div><i className="is-late" /><span>延期补交</span><strong>{summary.late}</strong></div><div><i className="is-missing" /><span>未提交</span><strong>{summary.missing}</strong></div></div>{summary.abnormalCount ? <div className="dashboard-weekly-overview__alert"><WarningCircle size={14} weight="fill" />异常周报需重点跟进 <strong>{summary.abnormalCount} 份</strong></div> : null}</div></div></section>;
}

export function PersonnelDashboard({ activeRole = "ceo", toolbar }) {
  ...
  const trendSeries = cycles.slice(0, 3).reverse().map((itemCycle) => {
    const records = filterByRole(withPeriodChanges(dashboardPerformanceRecords, itemCycle, getPreviousCycle(cycles, itemCycle)), roleId);
    const effective = records.filter((item) => item.finalScoreEffective);
    const average = effective.length ? Math.round((effective.reduce((sum, item) => sum + item.finalScore, 0) / effective.length) * 10) / 10 : 0;
    return { cycle: itemCycle, average, completionRate: records.length ? Math.round((effective.length / records.length) * 100) : 0 };
  });
  ...
  return <div className="admin-page personnel-dashboard-v2">
    <section className="dashboard-title"><div className="dashboard-title__heading"><span className="dashboard-title__icon"><ChartLineUp size={22} weight="duotone" /></span><div><span className="dashboard-title__eyebrow">实时更新</span><h2>人员绩效与周报数据看板</h2><p>集中查看团队绩效、周报提交与人员波动，快速定位重点风险</p></div></div><div className={`dashboard-title__meta ${toolbar ? "has-tabs" : ""}`}>{toolbar ? <div className="dashboard-title__tabs">{toolbar}</div> : null}<div className="dashboard-title__actions"><div className="dashboard-title__updated"><small>数据更新时间</small><strong>{dashboardMeta.updatedAt}</strong></div><button className="ghost-chip" onClick={() => setScopeOpen(true)} type="button"><Info size={16} />统计口径</button><button className="primary-btn" onClick={refresh} type="button">刷新数据</button></div></div></section>
