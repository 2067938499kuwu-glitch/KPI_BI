...
Model `加减分项` as a special independent assessment dimension in both HR template maintenance and Leader performance issuance. It contains multiple configurable category rows with a name, evaluation criteria, and data source, but no weight and no participation in the 100% weight validation; its overall score range is -10 to +10. During assessment, the first- and second-level reviewers score each adjustment category independently and the system combines those scores using the existing 60% / 40% reviewer ratio before adding the result directly to the weighted base score. All adjustment categories share one evidence area that accepts one or multiple actual file uploads. When every adjustment score at the current completed scoring result is 0, evidence is optional; when any adjustment score is non-zero, at least one shared evidence file is mandatory and submission must be blocked until it exists. Preserve these rules in score sheets, review flows, details, exports, audit records, and tests.
...
For performance appeals, do not download or upload an appeal-form attachment. Use one shared online `绩效申诉表` modeled on the supplied paper form: the employee enters appeal content and an evidence list directly, then HR, the employing-department leader, the comprehensive management center, and the performance committee fill only their corresponding sections as the same record moves through the workflow. Preserve legacy uploaded forms only for backward-compatible data migration; never require them for a new appeal.

In performance Details, every metric must show each applicable leader's name, score, and comment in addition to the composite and included scores. Clearly distinguish missing comments from roles that do not require a second-level review.
